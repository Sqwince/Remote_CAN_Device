...
//RPMsEffect RPMs_Left(leds, false, 0, 9, CRGB::Green, CRGB::Red, 0, 100, true, CRGB::Red, CRGB::Blue);
//RPMsEffect RPMs_Right(leds, true, 9, 9, CRGB::Green, CRGB::Red, 0, 100, true, CRGB::Blue, CRGB::Red);

BlinkEffect yellowFlag(leds, 1, 0, 18, CRGB::Green, CRGB::Blue);
...
void loop() {
  ...
  if (currentMillis - previousMillis >= refreshRateDelayInMillis) {
    ...
    bool state = false;
    if (rpmPercentage > 50) { state = true; }
    yellowFlag.update(state);
